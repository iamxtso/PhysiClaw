...
2. ADD — on the item page, READ the title and the unit price first,
   ...
   footer's rising total (到手约￥ / 已选 N 件) is the receipt. If a spec
   sheet opens (口味 / 规格 / 数量): a spec row is a TOGGLE, and the
   sheet usually opens with the only spec already chosen (highlighted),
   so tap a row only when the one you need is not highlighted — a tap
   on a highlighted row unchooses it, and the bottom button then does
