...
1. PICK — compare the listings from the results rows you can
   ...
   sales, low price; 百亿补贴 first. The item's unit word (一包,
   一箱, 一瓶) is how the buyer counts, not a pack size to match:
   a listing sold as one unit of the product fits (a case of
   milk, a bag of tissue packs); a bundle is two or more of that
   unit. A spec such as 5kg×4袋 or ×2 on a row below a title
...
2. ADD — on the detail page tap the footer's LEFT button,
   ...
   not your last action — decides the move:
     …起 (a RANGE) — no spec is chosen. Tap the option row
       matching the item: the single pack, never a ×2, 套餐 or
       搭配 row.
     ONE exact price for a spec you did NOT want — tap the row
       you want: that tap chooses, it does not unchoose.
     ONE exact price for the spec you want — it IS chosen. Do
       NOT tap an option row.
   An option row is a TOGGLE: tapping the row that is ALREADY
   chosen UNCHOOSES it. Read the header BEFORE each tap, and
   never tap the same option row twice in a row.
   Set the quantity the item names with the stepper's ＋ (one
