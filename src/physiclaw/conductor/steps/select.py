...
KIND_SCROLL = "select-scroll"


class SelectStep(Step[SelectNode]):
    kinds = frozenset({KIND_SCROLL})

    def __init__(self, walk: Walk, node: SelectNode) -> None:
    ...
    def open(self) -> Turn:
        ...
        if walk.activation is None:
            return walk.handover(
                f"select {self.node.id!r}: no playbooks to offer — the boot "
    ...
    def landed(self, kind: str) -> Turn:
        ...
        if wrong is not None:
            return self.walk.handover(
                f"select {self.node.id!r}: the history scroll did not land on "
