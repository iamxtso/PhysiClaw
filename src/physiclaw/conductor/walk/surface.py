"""The seam between the walk and its steps, both halves: `Step` — what
every route line kind implements and the walk drives — and `Walk`, what
a step may rely on of the program; `Steps` is the executors the walk is
handed at construction (`steps/table.py` builds it), so nothing under
`walk/` imports an executor.
...
from physiclaw.conductor.micro.decision import DecisionRequest, MicroOutcome
...
@dataclass(frozen=True)
class Steps:
    """The executors a walk dispatches to — one per node kind, keyed by
    the node's type, and the close step a completed walk ends with.
    Built once in `steps/table.py`, handed in by `drive/build.py`."""

    for_node: Mapping[type, "type[Step]"]
    close: "Callable[[Walk], Step]"
