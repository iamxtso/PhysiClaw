...
Text only: the mechanism (`calltable.py`) assembles the skeleton — role
sentence → reply contract → legend — and the rows there name these
(the tool legend included, one line per tool over `spec.calls`' names).
...
# micro calls' blocks (`micro.blocks.data_block`) and the handover brief's
...
# under its name. They arrive as DATA (`micro.blocks.data_block`) and never
