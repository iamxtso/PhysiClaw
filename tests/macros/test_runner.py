...
def _spec(text: str, name: str = "demo", **kw):
    """One macro FILE parsed. The fixtures carry the body; the header its
    place in `macros/` requires is added here, so the runner's cases read
    as steps."""
...
def _send_spec(open_text: str = OPEN_TEXT, send_text: str = SEND_TEXT):
    open_spec = _spec(open_text, "open")
    return _spec(send_text, "send", macros=lambda n: open_spec)
